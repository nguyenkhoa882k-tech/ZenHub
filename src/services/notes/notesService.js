...
class NotesService {
  ...
  async initialize() {
    ...
    if (this.storage.storageType === 'SQLITE') {
  ...
  async createNote(data = {}) {
    ...
    if (this.storage.storageType === 'SQLITE') {
      ...
      if (data.tags && data.tags.length > 0) {
        await this.addTagsToNote(noteId, data.tags);
      }

      // Tạo version đầu tiên
      await this.createNoteVersion(noteId, note.title, note.content, 1);
      ...
      await this.storage.setItem(`note_${noteId}`, fullNote);

      // Cập nhật danh sách notes
      const notesList = (await this.storage.getItem('notes_list')) || [];
      notesList.push(noteId);
      await this.storage.setItem('notes_list', notesList);
  ...
  async getNoteById(noteId) {
    if (this.storage.storageType === 'SQLITE') {
      ...
      const result = await this.storage.executeSql(sql, [noteId]);
      ...
      return await this.storage.getItem(`note_${noteId}`);
    }
  }
  ...
  async updateNote(noteId, updates) {
    ...
    if (this.storage.storageType === 'SQLITE') {
      ...
      await this.storage.setItem(`note_${noteId}`, updatedNote);
      return updatedNote;
    }
  }
  ...
  async deleteNote(noteId) {
    if (this.storage.storageType === 'SQLITE') {
      ...
      await this.storage.executeSql(sql, [noteId]);
    } else {
      const note = await this.getNoteById(noteId);
      if (note) {
        note.trashed = true;
        note.updated_at = new Date().toISOString();
        await this.storage.setItem(`note_${noteId}`, note);
      }
    }
  }
  ...
  async permanentDeleteNote(noteId) {
    if (this.storage.storageType === 'SQLITE') {
      await this.storage.transaction(async tx => {
        ...
        // Xóa tags
        await tx.executeSql('DELETE FROM note_tags WHERE note_id = ?', [
          noteId,
        ]);
        // Xóa versions
      ...
      await this.storage.removeItem(`note_${noteId}`);
      const notesList = (await this.storage.getItem('notes_list')) || [];
      const updatedList = notesList.filter(id => id !== noteId);
      await this.storage.setItem('notes_list', updatedList);
    }
  }
  ...
  async getAllNotes(options = {}) {
    ...
    if (this.storage.storageType === 'SQLITE') {
      ...
      for (const noteId of notesList) {
        const note = await this.storage.getItem(`note_${noteId}`);
        if (
          note &&
  ...
  async searchNotes(query, options = {}) {
    ...
    if (
      this.storage.storageType === 'SQLITE' &&
      ...
      const result = await this.storage.executeSql(sql, [query, limit]);
      const notes = [];
  ...
  async createTag(name, color = '#6B7280') {
    if (this.storage.storageType === 'SQLITE') {
      ...
      const result = await this.storage.executeSql(sql, [name, color]);
      return result.insertId;
    } else {
      const tags = (await this.storage.getItem('tags')) || {};
      ...
      await this.storage.setItem('tags', tags);
      return tagId;
    }
  }
  ...
  async getAllTags() {
    if (this.storage.storageType === 'SQLITE') {
      ...
      const result = await this.storage.executeSql(sql);
      const tags = [];
  ...
  async addTagsToNote(noteId, tagNames) {
    if (this.storage.storageType === 'SQLITE') {
      for (const tagName of tagNames) {
        ...
        let tagId = await this.createTag(tagName);
   ...
   */
  async updateNoteTags(noteId, tagNames) {
    if (this.storage.storageType === 'SQLITE') {
      // Xóa tất cả tags cũ
      await this.storage.executeSql('DELETE FROM note_tags WHERE note_id = ?', [
        noteId,
      ]);
   ...
   * Dọn dẹp versions cũ
   */
  async cleanupOldVersions(noteId) {
    if (this.storage.storageType === 'SQLITE') {
      const sql = `
   ...
   */
  async createNoteFromTemplate(templateId, data = {}) {
  ...
  async getStats() {
    if (this.storage.storageType === 'SQLITE') {
      ...
      const results = await Promise.all(
        queries.map(sql => this.storage.executeSql(sql)),
      );
   ...
   */
  async importData(jsonData, options = { merge: true }) {
    try {
      const data =
